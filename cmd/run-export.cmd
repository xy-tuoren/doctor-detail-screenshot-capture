...
echo If export coordinates are not calibrated, run cmd\run-export-calibrate.cmd first.
echo Exported files will be saved to the exports folder by default.
echo.
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%ROOT%\scripts\capture-doctor-details.ps1" -Mode Export -ListEntry Main
if errorlevel 1 (
  echo.
