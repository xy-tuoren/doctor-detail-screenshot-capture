...
- 科室：`professionalList` ← 「执业范围」（`,`/`，` → `;`）+ 「医师类别」→ `professionalType`。例外：单项执业范围为「公共卫生类别专业」时，无论医师类别为何，`professionalType` 一律为 `4`（公共卫生）；提交展平（`postable_body`）时再次校正，避免历史 `to_submit` 仍带错类型。
